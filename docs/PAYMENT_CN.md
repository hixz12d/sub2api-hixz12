...
- **站内购买入口**：默认开启。关闭后隐藏用户侧充值/订阅菜单、拦截购买页直达，并拒绝新建订单。
- **购买入口随 PerPay 联动**：默认关闭，以兼容现有站点。开启后，至少有一个 PerPay 服务商处于启用状态时才开放入口；全部停用或删除后入口自动关闭。手动关闭入口始终优先。

联动依据后台服务商的启用状态，不探测 PerPay 服务器在线情况，也不修改 PerPay 自带商店的营业设置。订单历史、已有订单查单/恢复和支付回调沿用原有处理逻辑，关闭入口不取消或改写已有订单。保存设置或切换服务商后，当前管理会话会刷新入口状态；其他已打开会话在刷新页面或再次进入购买页时读取新状态。后端创建订单接口始终检查最新开关。

支持 PerPay 0.2.2 的原生签名 API。添加 `PerPay` 服务商，填写 HTTPS API 基础地址、业务 `API Secret`、通知 `Webhook Secret`，并将前台支付宝来源设为 `PerPay 支付宝`。这些密钥与支付宝应用私钥不同。PerPay 后台须开启业务通知，允许来源填写 Sub2API 的 HTTPS Origin；回调路径为 `/api/v1/payment/webhook/perpay`。

- 用户打开 PerPay 收银台，必须按其显示的含尾数金额付款。余额充值订单的尾数也按下单时的充值倍率计入余额，不作为手续费扣除。例如倍率为 1、手续费为 0 时，实付 1.02 元就充值 1.02 余额。订单会记录实际应付金额与预计到账余额；订阅订单保持套餐权益不变。
- 服务端验证原始回调签名、毫秒时间戳和事件 ID，再主动查单核对当前状态、订单号、币种、应付及实收金额；仅 `CONFIRMED` 且实收等于应付时充值。
- 查单同样支持补单。重复通知复用现有订单状态 CAS 和充值幂等机制；旧确认通知会重新查询最新状态，不能把 `DISPUTED` 回退为已付款。争议会写入订单审计日志，需要管理员核对已发放权益。
- 订单倒计时使用 PerPay 的实际过期时间；取消订单会尝试关闭上游收银台。静态码仍可能收到超时付款，必须在 PerPay 后台处理异常收入。
- 不支持自动退款。PerPay 管理员“退款标记”不会实际转账，也不会同步回 Sub2API；退款与余额回收需人工核对处理。
- 回跳地址必须属于 PerPay 配置的通知来源；通过其他域名访问 Sub2API 时，会回到服务商配置中的规范地址。
