...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingFilter().Enabled {
			source := availableModels
			if len(source) == 0 {
				source = defaultModelIDsForPlatform(service.PlatformComposite)
			}
			writeAllowlistedModelsList(c, service.PlatformComposite, apiKey.Group.ModelListingFilter().FilterForListing(source))
			return
		}
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingFilter().Enabled {
		...
		writeAllowlistedModelsList(c, platform, apiKey.Group.ModelListingFilter().FilterForListing(source))
		return
	}
...
func (h *GatewayHandler) codexModelIDsForGroup(ctx context.Context, group *service.Group, platformOverride string) []string {
	...
	if platform == service.PlatformComposite {
		...
		if group.ModelListingFilter().Enabled {
			source := availableModels
			if len(source) == 0 {
				source = fallbackModels
			}
			return group.ModelListingFilter().FilterForListing(source)
		}
		if len(availableModels) > 0 {
	...
	if group.ModelListingFilter().Enabled {
		return group.ModelListingFilter().FilterForListing(modelListingSource(platform, availableModels, fallbackModels))
	}
	if len(availableModels) > 0 {
		return availableModels
	}
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	models := antigravity.DefaultModels()
	if apiKey, ok := middleware2.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelListingFilter().Enabled {
		filtered := make([]antigravity.ClaudeModel, 0, len(models))
		for _, model := range models {
			if apiKey.Group.ModelListingFilter().Allows(model.ID) {
