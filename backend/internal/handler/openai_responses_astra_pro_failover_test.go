...
// astraProForbiddenResponse is request-scoped under the fork's retry budget;
// it must not rotate credentials without evidence of an account-scoped failure.
...
func astra503() *http.Response {
	return &http.Response{
		StatusCode: http.StatusServiceUnavailable,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(bytes.NewBufferString(`{"error":{"type":"server_error","message":"Service unavailable"}}`)),
	}
}
...
// 503 then a second OAuth account succeeding: wire bodies keep pro+max/model and the
// winning account's structured reasoning.mode=pro reaches the client unchanged.
func TestOpenAIGatewayHandlerResponses_AstraProFirst503SecondSucceeds(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstream := newAstraProCapturedUpstream(astra503(), astra200())
...
// Both OAuth accounts return 503: exhaustion must keep the pro+max/model contract
// and surface 502 without downgrading the requested reasoning mode.
func TestOpenAIGatewayHandlerResponses_AstraProBoth503NoDowngrade(t *testing.T) {
	gin.SetMode(gin.TestMode)

	upstream := newAstraProCapturedUpstream(astra503(), astra503())
	handler := newOpenAIResponsesFailoverTestHandler(t, upstream)
	c, rec := newAstraProFailoverContext(t, astraProRequestBody)
	...
	assertAstraProWire(t, urls, bodies)
	assertAstraProAccountSwitch(t, accountIDs)
	require.NotEqual(t, http.StatusOK, rec.Code, "all-503 must never surface a 200 to the client")
	require.Equal(t, http.StatusBadGateway, rec.Code)
	require.Equal(t, "upstream_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
}

// Preserve the local fail-closed rule for an unclassified 403, even if a second
// account could succeed. Upstream failover must not bypass the retry budget.
func TestOpenAIGatewayHandlerResponses_AstraPro403DoesNotRotate(t *testing.T) {
	gin.SetMode(gin.TestMode)
