...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	filterGeminiModels := func(models []gemini.Model) []gemini.Model {
		if apiKey.Group == nil || !apiKey.Group.ModelListingFilter().Enabled {
			return models
		}
		filtered := make([]gemini.Model, 0, len(models))
		for _, model := range models {
			if apiKey.Group.ModelListingFilter().Allows(model.Name) {
	...
	if apiKey.Group != nil && apiKey.Group.ModelListingFilter().Enabled {
		if filtered, dropped, ok := filterUpstreamGeminiModelsBody(res.Body, apiKey.Group.ModelListingFilter()); ok && dropped {
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	for {
		...
		if decision := h.checkSelectedAccountContentModeration(c, reqLog, apiKey, authSubject, account, service.ContentModerationProtocolGemini, modelName, body); decision != nil && !decision.AllowNextStage {
