...
func isOpenAICompatMessagesBridgeBody(body []byte) bool {
	...
	if bytes.Contains(body, []byte(openAICompatClaudeCodeTodoGuardMarker)) || bytes.Contains(body, []byte(legacyOpenAICompatClaudeCodeTodoGuardMarker)) {
		return true
	}
...
func isOpenAICompatMessagesBridgeRequestBody(reqBody map[string]any) bool {
	...
	if input, ok := reqBody["input"].([]any); ok && (inputContainsText(input, openAICompatClaudeCodeTodoGuardMarker) || inputContainsText(input, legacyOpenAICompatClaudeCodeTodoGuardMarker)) {
		return true
	}
