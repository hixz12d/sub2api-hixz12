...
func TestOpenAIGatewayService_BuildOpenAIWSHeadersUsesAPIKeyLegacySessionContract(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	require.Equal(t, "session-ws", headers.Get("session_id"))
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthOriginatorCompatibility(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NotNil(t, result)
			requireOfficialCodexOutboundHeaders(t, captureDialer.lastHeaders)
		})
	}
}
...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, gjson.Get(headerTurnMetadata, "turn_id").String(), captureDialer.lastHeaders.Get("x-client-request-id"))
