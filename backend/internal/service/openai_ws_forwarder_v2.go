...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	promptCacheKey := strings.TrimSpace(rawPromptCacheKey)
	if promptCacheKey == "" {
	...
	transportScopeKey := s.finalizeCodexAttemptWSWire(c, wsHeaders, payloadAsJSONBytes(payload))
	logOpenAIWSModeDebug(
		...
		openAIWSConversationHeaderValueForLog(wsHeaders),
