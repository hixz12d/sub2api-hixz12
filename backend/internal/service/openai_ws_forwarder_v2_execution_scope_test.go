...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	forward := func(sessionID, body string) (*gin.Context, []byte) {
		...
		raw := []byte(body)
		// Local handshake isolation retires the previous scope's socket. Each
		// dial must return a fresh connection rather than the closed test object.
		pool.setClientDialerForTest(&openAIWSCaptureDialer{
			conn:      &openAIWSCaptureConn{events: [][]byte{completed("resp_" + sessionID)}},
			handshake: handshake,
		})
