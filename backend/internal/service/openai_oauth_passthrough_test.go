...
func TestOpenAIGatewayService_OAuthMessagesBridgeDoesNotInjectDefaultInstructions(t *testing.T) {
	...
	requireOfficialCodexOutboundHeaders(t, upstream.lastReq.Header)
}
...
func TestOpenAIGatewayService_OAuthDoesNotForwardSub2APIUserAgent(t *testing.T) {
	gin.SetMode(gin.TestMode)

	for _, name := range []string{"passthrough", "ordinary"} {
		t.Run(name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(nil))
			c.Request.Header.Set("User-Agent", "sub2api/1.0")
			c.Request.Header.Set("originator", "sub2api")

			resp := &http.Response{
				StatusCode: http.StatusOK,
				Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_sub2api_ua"}},
				Body:       io.NopCloser(strings.NewReader("data: [DONE]\n\n")),
			}
			upstream := &httpUpstreamRecorder{resp: resp}
			svc := &OpenAIGatewayService{
			...
			account := &Account{
				...
				Credentials: map[string]any{"access_token": "oauth-token", "chatgpt_account_id": "chatgpt-acc"},
				Extra: map[string]any{
					"openai_passthrough": name == "passthrough",
				},
				Status:         StatusActive,
				Schedulable:    true,
				RateMultiplier: f64p(1),
			...
			_, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.2","stream":false,"input":"hi"}`))
			require.NoError(t, err)
			require.NotNil(t, upstream.lastReq)
			requireOfficialCodexOutboundHeaders(t, upstream.lastReq.Header)
		})
...
func TestOpenAIGatewayService_CodexFingerprintHTTPTransformedHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, gjson.Get(headerTurnMetadata, "turn_id").String(), upstream.lastReq.Header.Get("x-client-request-id"))
...
func TestOpenAIGatewayService_CodexFingerprintCompactFinalizesHeaderAndBodyIdentity(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIGatewayService_CodexFingerprintMessagesBridgeFinalizesBodyPromptCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIGatewayService_OAuthPassthrough_AllowTimeoutHeadersWhenConfigured(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	require.Empty(t, upstream.lastReq.Header.Get("x-stainless-timeout"), "OAuth finalizer must strip client timeout metadata")
	require.Empty(t, upstream.lastReq.Header.Get("X-Test"))
}
