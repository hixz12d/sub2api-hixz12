...
func FilterCodexModelIDsForGroup(modelIDs []string, group *Group) []string {
	...
	if group != nil && group.ModelListingFilter().Enabled {
		for _, modelID := range group.ModelListingFilter().Models {
	...
	if group != nil && group.ModelListingFilter().Enabled {
		filtered = group.ModelListingFilter().FilterForListing(filtered)
	}
	return filtered
}
...
func (s *OpenAIGatewayService) BuildGroupConfiguredCodexModelsManifest(
	...
	body, _, err = mergeConfiguredCodexModelsManifest(
		body,
		nil,
		group.ModelListingFilter().Models,
		group.ModelListingFilter().Enabled,
	)
	if err != nil {
...
func (s *OpenAIGatewayService) MergeGroupConfiguredCodexModels(
	...
	if group.CodexModelsManifestConfig.Enabled && group.ModelListingFilter().Enabled {
		body, err = orderPinnedCodexModelsBySelection(body, group.ModelListingFilter())
		if err != nil {
...
func openAIConfiguredCodexModelIDsForGroup(accounts []Account, group *Group) []string {
	...
	seen := make(map[string]struct{}, len(models)+len(group.ModelListingFilter().Models))
	for _, modelID := range models {
		seen[modelID] = struct{}{}
	}
	for _, selectedModel := range group.ModelListingFilter().Models {
