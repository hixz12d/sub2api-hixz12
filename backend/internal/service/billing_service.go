...
func (s *BillingService) initFallbackPricing() {
	...
	// Operator-specified Astra card (USD/token): $10/$12.5/$2/$50 per MTok.
	// Fast/priority remains 2x standard; existing long-context gates are unchanged.
	s.fallbackPrices["gpt-6-astra"] = &ModelPricing{
		...
		CacheReadPricePerTokenPriority:     4e-6,
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	usesCacheWritePremium := isOpenAIGPT56Model(normalized) || isOpenAIGPT6AstraModel(normalized) || isOpenAIGPT6SolLunaModel(normalized)
	...
	needsOpus55FastMultiplier := isClaudeOpus55Model(model) && pricing.FastMultiplier == nil
	...
	if !needsCacheCreationPolicy && fastRatio <= 0 && !needsOpus55FastMultiplier && !needsGrok46Policy {
		return pricing
	}
	cloned := *pricing
