...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool, pricingAt time.Time) *ModelPricing {
	...
	if !isOpenAICacheWritePremiumModel && fastRatio <= 0 && !isGrok46 && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	...
	if !needsGrok46Policy && !needsCacheCreationPolicy && fastRatio <= 0 && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	cloned := *pricing
	...
	if isOpenAICacheWritePremiumModel && !cloned.CacheCreationPriceExplicit {
