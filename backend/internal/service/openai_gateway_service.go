...
const (
	...
	// 该后缀只用于无账号兜底和 ForceCodexCLI；有 OAuth 账号时改走平台指纹库。
...
var openaiAllowedHeaders = map[string]bool{
	...
	"user-agent":              true,
	"originator":              true,
	legacyCodexSessionHeader:  true,
	codexSessionHeader:        true,
	legacyCodexThreadHeader:   true,
	codexThreadHeader:         true,
...
type OpenAIForwardResult struct {
	...
	Usage           OpenAIUsage
	// Independent upstream observation; never used to calculate customer charges.
	...
	modelRoutePauseHandled       bool // forwarding/WS hook deduplication, before async usage
...
type OpenAIGatewayService struct {
	...
	openAIAffinityRepo    OpenAIAffinityRepository
...
func NewOpenAIGatewayService(
	...
	if affinityRepo, ok := accountRepo.(OpenAIAffinityRepository); ok {
		svc.openAIAffinityRepo = affinityRepo
	}
