...
func TestExtractContentModerationKeywordText_Boundaries(t *testing.T) {
	for _, tc := range []struct{ protocol, body string }{
		...
		t.Run(tc.protocol+tc.body, func(t *testing.T) {
			// This fork audits the latest real user input across tool/assistant turns.
			// Tool results and assistant content must still stay outside that boundary.
			require.Equal(t, "old", extractContentModerationKeywordText(tc.protocol, []byte(tc.body)))
		})
	}
}
