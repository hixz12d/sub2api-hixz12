...
func TestFetchCodexModelsManifestAPIKeyCacheKeyIsolatesRequestIdentity(t *testing.T) {
	...
	differentProxy.Proxy = &Proxy{ID: proxyID, Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: StatusActive}
...
func TestFetchCodexModelsManifestAPIKeyUsesOfficialOpenAIModelsEndpoint(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, codexDefaultVersion, parsedURL.Query().Get("client_version"), "official manifests use the enforced outbound identity")
			models := decodeCodexManifestModels(t, manifest.Body)
			require.Len(t, models, 1)
