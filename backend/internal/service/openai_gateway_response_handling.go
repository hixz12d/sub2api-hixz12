...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	for {
		...
		case <-keepaliveCh:
			...
			if stageFirstOutput {
				// Bypass attempt-local buffered frames. Stable SSE headers may be committed,
				// but account headers remain private until semantic output.
				n, err := w.Write([]byte(":\n\n"))
			...
			// Finish any oversized buffered preamble before adding a comment so bytes
			// cannot interleave as an invalid SSE frame.
