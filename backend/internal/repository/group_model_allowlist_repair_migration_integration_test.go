...
// Repair must not turn an old display preference into an admission restriction.
func TestMigration236PreservesLegacyModelsListConfigColumn(t *testing.T) {
	tx := testTx(t)
...
`).Scan(&groupID))
	...
	// The newly added admission column stays disabled; display data is untouched.
	...
	require.NoError(t, tx.QueryRowContext(ctx,
		"SELECT models_list_config::text FROM groups WHERE id = $1", groupID).Scan(&display))
...
func TestMigration236KeepsBothColumnsIndependent(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	...
	// An empty admission policy must remain empty, even with an enabled display list.
	var staleID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
