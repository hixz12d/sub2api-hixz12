...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if err := scanner.Scan(
		...
		&nativeCompactionV2,
		&createdAt,
		&monitor.RequestOrigin,
		&monitor.MonitorObservationVersion,
		&monitor.MonitorInputTokensTotal,
		&monitor.MonitorCacheReadTokens,
		&monitor.MonitorVisibleOutputTokens,
		&monitor.MonitorGenerationMs,
		&monitor.MonitorOutputTPSMilli,
		&monitor.MonitorTPSMethod,
		&monitor.MonitorFirstVisibleMs,
	); err != nil {
		return nil, err
	}

	log := &service.UsageLog{
		RequestOrigin:              monitor.RequestOrigin,
		MonitorObservationVersion:  monitor.MonitorObservationVersion,
		MonitorInputTokensTotal:    monitor.MonitorInputTokensTotal,
		MonitorCacheReadTokens:     monitor.MonitorCacheReadTokens,
		MonitorVisibleOutputTokens: monitor.MonitorVisibleOutputTokens,
		MonitorGenerationMs:        monitor.MonitorGenerationMs,
		MonitorOutputTPSMilli:      monitor.MonitorOutputTPSMilli,
		MonitorTPSMethod:           monitor.MonitorTPSMethod,
		MonitorFirstVisibleMs:      monitor.MonitorFirstVisibleMs,
		ID:                         id,
		UserID:                     userID,
		APIKeyID:                   apiKeyID,
		AccountID:                  accountID,
