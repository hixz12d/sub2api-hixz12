...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, privacyClientFactory, configConfig)
	...
	oAuthSyncRepository := repository.NewOAuthSyncRepository(db)
	oAuthSyncService := service.ProvideOAuthSyncService(oAuthSyncRepository, accountRepository, compositeTokenCacheInvalidator, schedulerSnapshotService, privacyClientFactory, proxyRepository, configConfig)
	...
	openAIQuotaService := service.ProvideOpenAIQuotaService(accountRepository, proxyRepository, openAITokenProvider, privacyClientFactory, openAIReferralClient, openAIGatewayService, configConfig)
	usageCache := service.NewUsageCache()
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, configConfig, usageCache, identityCache, tlsFingerprintProfileService, openAIGatewayService)
	...
	benchmarkRegistryRepository := repository.NewBenchmarkRegistryRepository(db)
	benchmarkService := service.ProvideBenchmarkService(configConfig, benchmarkRegistryRepository, userRepository)
	benchmarkHandler := admin.NewBenchmarkHandler(benchmarkService)
	...
	questionReviewRepository := repository.NewQuestionReviewRepository(db)
	questionReviewService := service.NewQuestionReviewService(questionReviewRepository, userRepository)
	llmDetectorRepository := repository.NewLLMDetectorRepository(db)
	monitorJobRepository := repository.NewMonitorJobRepository(db)
	capabilityExecutor, err := service.ProvideCapabilityExecutor(configConfig, monitorJobRepository, benchmarkRegistryRepository, apiKeyRepository, userRepository, groupRepository, accountRepository, settingService, httpUpstream, concurrencyService)
	if err != nil {
		return nil, err
	}
	...
	monitorProbeExecutor := service.ProvideMonitorProbeExecutor(configConfig, monitorJobRepository, accountRepository, groupRepository, httpUpstream, settingService, concurrencyService)
	monitorJobRuntime, err := service.ProvideMonitorJobRuntime(monitorJobRepository, settingService, monitorProbeExecutor, capabilityExecutor)
	if err != nil {
		return nil, err
	}
	channelMonitorV2Aggregator := service.ProvideChannelMonitorV2Aggregator(channelMonitorV2Repository, db, settingService)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, claudeCodeVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, oAuthSyncService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, monitorJobRuntime, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager)
