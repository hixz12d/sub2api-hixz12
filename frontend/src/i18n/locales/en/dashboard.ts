export default {
  connectionDiagnostics: {
    title: 'Configuration and connection checks',
    scope: 'Check this URL and key. Browser results reflect this browser’s network and do not replace testing from your client computer.',
    configurationReady: 'The URL format is valid and a key is provided. The installed client configuration has not been verified.',
    model: 'Test model', clientVersion: 'Client version (optional)',
    compatibility: 'Run codex --version to check your version. If WebSocket configuration is unsupported, upgrade or select the standard Codex setup. Follow the restart instructions after changing authentication mode.',
    checkModels: 'Check authentication and models', cancel: 'Cancel check',
    models: 'Model list and authentication', modelAvailability: 'Selected model', httpStream: 'HTTP streaming request',
    available: 'Present in the model list', modelMissing: 'Not in the model list; check the model name or group permissions',
    idle: 'Not checked', running: 'Checking', passed: 'Passed', failed: 'Failed', cancelled: 'Cancelled',
    paidNotice: 'I agree to send one small model request, which may be billed at current rates. This check does not run tools or modify files.',
    checkStream: 'Send a streaming test request',
    localTitle: 'Check WebSocket from your client computer',
    localHint: 'Browsers cannot set the required WebSocket authentication header. Run the downloaded script on your client computer with Node.js 20 or newer. By default it checks models and the handshake without generation. It does not import proxy settings from your client application. A successful handshake does not verify a complete model request.',
    download: 'Download diagnostic script', copyCommand: 'Copy run command', copyReport: 'Copy check results',
    commandHint: 'Run the copied command in the script directory (PowerShell on Windows). It contains your key; do not share it. Script output excludes the key. Add --stream manually to test generation in the terminal; this may incur charges.',
    invalidUrl: 'Enter a full HTTP/HTTPS base URL without a username, password, query or fragment.',
    endpointUrl: 'Use a base URL such as https://example.com/v1, not a specific /responses or /models endpoint.',
    missingKey: 'Provide an API key first.',
    auth: 'Gateway key authentication failed. Check that the key is correct, enabled and unexpired.',
    rateLimit: 'A request-rate or concurrency limit was reached. Retry later.',
    httpError: 'The endpoint returned an error. Check its HTTP status and your error request records.',
    invalidModels: 'The response is not a valid model list. Check the base URL and reverse proxy.',
    responseTooLarge: 'The response exceeded the diagnostic size limit; reading was stopped.',
    notStream: 'The endpoint did not return SSE. Check the URL and proxy configuration.',
    incompleteStream: 'The connection ended without a complete response terminal event.',
    invalidStream: 'The stream contained invalid event data.',
    upstreamRejected: 'The model returned a failure event. Check your error request records.',
    timeout: 'The check timed out. Model-list checks allow 15 seconds; streaming tests allow 45 seconds.',
    browserNetwork: 'The browser could not complete the request. Network, TLS, CORS restrictions or redirects may be involved. Run the local diagnostic script to distinguish these causes.'
  },
  dashboard: {
    title: 'Dashboard',
  ...
  usage: {
    ...
    errors: {
      ...
      detail: {
        ...
        copyReport: 'Copy troubleshooting details',
        reportHint: 'Copies the error ID, time, key name, model and client details. Excludes credentials, request bodies and raw error messages.',
        advice: {
          auth: 'Check that the gateway API key in your client is correct, enabled and unexpired. Restart your client as instructed after changing authentication mode.',
          quota: 'Check your balance, key quota and subscription validity, then retry after resolving the applicable limit.',
          concurrency: 'The active-request or waiting-queue limit was reached. Wait for existing tasks to finish or reduce parallel tasks before retrying.',
          rate_limit: 'A request-rate or concurrency limit was reached. Wait before retrying and reduce simultaneous requests.',
          firstOutput: 'The model did not start producing output before the deadline. Retry later; copy these details if this happens repeatedly.',
          timeout: 'The request timed out. Retry later; if you received a partial answer, check the task state first to avoid duplicate execution.',
          invalid_request: 'Check model availability, request parameters and client compatibility. Use the setup guide to review your configuration.',
          service_unavailable: 'No service route is currently available. Retry later and share these details if the problem persists.',
          upstream: 'The upstream request failed. Recreating your gateway API key is usually unnecessary. Retry later and share these details if the problem persists.',
          internal: 'The platform encountered an error while processing the request. Retry later and share these details if it persists.',
          cyber: 'A safety policy restricted this request. Review the request and service rules; share these details if you need clarification.',
          other: 'The cause is not yet clear. Keep your current configuration and copy these details for further investigation.'
        },
      },
    },
  },
