export default {
  connectionDiagnostics: {
    title: '配置检查与连接诊断',
    scope: '检查当前地址和 Key 的连接情况。浏览器测试反映当前浏览器所在网络，不能代替客户端电脑的测试。',
    configurationReady: '地址格式正常，已提供 Key；尚未验证实际客户端配置。',
    model: '测试模型', clientVersion: '客户端版本（选填）',
    compatibility: '运行 codex --version 查看版本。若客户端不支持 WebSocket 配置，请升级或选择普通 Codex 配置。改变认证模式后请按提示重启客户端。',
    checkModels: '检查鉴权与模型列表', cancel: '取消检查',
    models: '模型列表与鉴权', modelAvailability: '所选模型', httpStream: 'HTTP 流式请求',
    available: '已在模型列表中', modelMissing: '模型列表中未找到，请检查模型名称或分组权限',
    idle: '未检查', running: '检查中', passed: '通过', failed: '失败', cancelled: '已取消',
    paidNotice: '我同意发送一次简短的模型请求，可能按当前价格计费。此检查不会运行工具或修改文件。',
    checkStream: '发送流式测试请求',
    localTitle: '在客户端电脑上检查 WebSocket',
    localHint: '浏览器无法为 WebSocket 设置所需的认证头。下载脚本后，在客户端电脑运行，需要 Node.js 20 或更高版本。默认只检查模型列表与握手，不发起模型生成；也不会读取客户端软件内部的代理设置。握手成功不代表整轮模型请求成功。',
    download: '下载诊断脚本', copyCommand: '复制运行命令', copyReport: '复制检查结果',
    commandHint: '在脚本所在目录运行复制的命令（Windows 使用 PowerShell），命令包含当前 Key，请勿公开。脚本输出不包含 Key；如需在终端测试流式生成，可自行添加 --stream，此操作可能计费。',
    invalidUrl: '请输入完整的 HTTP/HTTPS 基础地址，不要包含用户名、密码、查询参数或片段。',
    endpointUrl: '这里需要基础地址，例如 https://example.com/v1，不要填写 /responses 或 /models 等具体接口。',
    missingKey: '请先提供 API Key。',
    auth: '本站 Key 鉴权失败，请检查 Key 是否正确、启用且未过期。',
    rateLimit: '请求受到频率或并发限制，请稍后重试。',
    httpError: '接口返回错误。请结合 HTTP 状态码和错误请求记录排查。',
    invalidModels: '响应不是有效的模型列表，请检查基础地址和反向代理配置。',
    responseTooLarge: '响应超过诊断大小限制，已停止读取。',
    notStream: '接口没有返回 SSE 流式响应，可能是地址或代理配置问题。',
    incompleteStream: '连接结束前没有收到完整的响应结束事件。',
    invalidStream: '收到的流式数据格式不正确。',
    upstreamRejected: '模型请求返回失败事件，请查看错误请求记录。',
    timeout: '诊断等待超时。模型列表最多等待 15 秒，流式测试最多等待 45 秒。',
    browserNetwork: '浏览器无法完成请求，可能涉及网络、TLS、跨域限制或地址重定向。请运行客户端诊断脚本进一步区分。'
  },
  dashboard: {
    title: '仪表盘',
  ...
  usage: {
    ...
    errors: {
      ...
      detail: {
        ...
        nextStep: '建议如何处理',
        copyReport: '复制故障信息',
        reportHint: '仅复制错误编号、时间、Key 名称、模型和客户端等信息，不包含密钥、请求正文或原始报错。',
        advice: {
          auth: '检查客户端使用的本站 API Key 是否正确、启用且未过期。更换认证方式后，请按配置说明重启客户端。',
          quota: '检查余额、Key 额度及订阅有效期，处理对应限制后再重试。',
          concurrency: '当前请求或排队数量达到限制。等待已有任务结束，或减少同时运行的任务后重试。',
          rate_limit: '请求触发了频率或并发限制。稍后重试，并减少同一时间发起的请求。',
          firstOutput: '模型未在规定时间内开始输出。稍后重试；若经常发生，请复制故障信息反馈。',
          timeout: '请求等待超时。稍后重试；如果已收到部分回答，请先检查任务状态，避免重复执行。',
          invalid_request: '检查所选模型、请求参数及客户端配置是否兼容。可使用接入向导重新核对配置。',
          service_unavailable: '当前没有可用的服务线路。稍后重试，持续失败时请提供故障信息。',
          upstream: '上游请求失败，通常无需重新创建本站 API Key。先稍后重试，持续失败时请提供故障信息。',
          internal: '平台处理请求时发生错误。稍后重试，持续失败时请提供故障信息。',
          cyber: '请求受到安全策略限制。请检查请求内容和服务使用规则，有疑问时提供故障信息反馈。',
          other: '暂时无法确定处理方式。请保留当前配置，复制故障信息以便进一步排查。'
        },
      },
    },
  },
