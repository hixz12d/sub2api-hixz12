export default {
    ...
    riskControl: {
      ...
      logsFailed: '加载审核记录失败',
      saved: '内容审计配置已保存',
      savedWithScope: '内容审计配置已保存（{scope}）',
      refresh: '刷新',
      config: '内容审计配置',
      ...
      auditScope: '审计范围',
      auditScopeHint: '选择审计全部分组、指定分组，或仅审计实际调度到指定上游账号的请求。',
      selectedGroups: '指定分组',
      selectedAccounts: '指定账号',
      selectedGroupCount: '已选 {count} 个分组',
      selectedAccountCount: '已选 {count} 个账号',
      selectedAccountsHint: '当前审计指定上游账号',
      savedScopeTargetsHint: '以下为当前生效的审计对象，保存后会直接显示在风控中心首页。',
      selectedAccountsRequired: '指定账号模式至少需要选择 1 个账号',
    ...
    channelMonitor: {
      ...
      form: {
        ...
        primaryModel: '主模型',
        primaryModelPlaceholder: '选择或搜索模型',
        useCustomModel: '使用',
        extraModels: '附加模型',
        extraModelsPlaceholder: '选择或回车添加附加模型',
        groupName: '分组名称',
        groupNamePlaceholder: '可选，用于在用户视图中聚合显示',
        intervalSeconds: '检测间隔 (秒)',
        intervalSecondsHint: '范围：15 - 9600 秒（最长约 2 小时 40 分）',
        jitterSeconds: '随机抖动 (± 秒)',
        jitterSecondsHint: '建议 200。每次检测在间隔基础上正负随机偏移该秒数，0 表示固定间隔；需满足 间隔 - 抖动 ≥ 15 秒',
        enabled: '启用监控',
        kindRequired: '请选择供应商'
      },
      runResultTitle: '检测结果',
