export default {
    accounts: {
      ...
      createTemplate: {
        title: '配置模板',
        hint: '把代理、WS、Codex指纹、并发、分组等创建时常改的默认值存成快照。选中后会写入当前表单，仍可再改。',
        placeholder: '选择模板',
        none: '不使用模板',
        save: '保存当前',
        saveAs: '另存为',
        ...
        saveAsTitle: '另存为模板',
        name: '模板名称',
        namePlaceholder: '例如 Team 轮转',
        includeGroups: '包含分组',
        includeGroupsHint: '关闭后这套模板不记录、也不覆盖分组。',
        isDefault: '设为该平台/类型的默认模板',
        isDefaultHint: '打开添加账号并切到对应类型时自动套用。',
        applyGroups: '同时应用分组',
        applyGroupsHint: '取消勾选后只写入代理、WS、指纹等字段，不动当前分组。',
        preview: '将写入：{summary}',
        defaultOption: '{name} （默认）',
        defaultBadge: '默认',
        setDefault: '设为默认',
        saveSuccess: '模板已保存',
        saveFailed: '保存模板失败',
      ...
      quotaExemptModels: '限额后仍允许的模型',
      quotaExemptModelsPlaceholder: '例如：gpt-4o, gpt-4o-mini',
      quotaExemptModelsHint: '支持逗号或换行分隔。达到账号额度后，仅这些精确匹配的模型仍可调用。',
      ...
      types: {
        ...
        responsesApi: 'OpenAI 兼容 API（Responses / Completions）',
      ...
      bulkEdit: {
        longContextParentRequired: '修改长上下文计费时，请选择对应的 OAuth 父账号。',
        longContextShadowHint: '影子账号继承 OAuth 父账号的长上下文计费设置。',
        successWithInherited: '已更新 {count} 个账号，{inherited} 个影子账号继承父账号设置。',
        partialSuccessWithInherited: '已更新 {success} 个账号，{failed} 个失败，{inherited} 个影子账号继承父账号设置。',
        title: '批量编辑账号',
      ...
      openai: {
        ...
        forceStoreFalse: '强制 store=false（Codex RS 兼容）',
        forceStoreFalseDesc: '仅此 API Key 账号：普通 Responses 请求强制禁用服务端存储，HTTP / WS 均生效；不修改 compact 请求。客户端仍需提供完整上下文。',
        ...
        completionsApiHint:
          '本网关同时支持 OpenAI 传统文本补全请求 /v1/completions，并复用同一套兼容调度链路。',
        endpointCapabilities: '端点能力',
        endpointCapabilitiesDesc:
          '用于调度筛选。文本端点会跟随上方 Responses API 支持显示为 Responses、Chat Completions 或自动模式；传统 /v1/completions 复用同一文本能力；Embeddings 独立控制 /v1/embeddings。',
        capabilityResponses: 'Responses',
        capabilityTextAuto: 'Responses / Chat Completions / Completions（自动）',
        capabilityResponsesAuto: 'Responses（自动探测）',
        capabilityChatCompletions: 'Chat Completions / Completions',
        capabilityChatCompletionsAuto: 'Chat Completions / Completions（自动探测）',
        ...
        codexRelayKernelSectionTitle: 'Codex 协议转发内核（Relay Kernel）',
        codexRelayKernelSectionDesc: '当前为固定版本兼容档案；原生客户端传输一致性尚未验证。',
        codexRelayMode: '中继架构模式',
        codexRelayModeDesc: 'Legacy 为传统透传与轻量收敛；Relay Kernel 启用完整的会话隔离、Wire-ID 桥接与协议保真度内核。',
        codexRelayModeLegacy: 'Legacy（传统兼容）',
        codexRelayModeKernel: 'Relay Kernel（保真中继内核）',
        codexRelayKernelBadge: 'Kernel 已选择',
        codexIdentityPolicyVersion: '会话身份策略版本',
        codexIdentityPolicyVersionDesc: '选择上游会话与 Wire-ID 隔离映射策略。v1 为传统粗粒度；v2 强化每会话独立性与上下文安全。Relay Kernel 模式下强制且锁定为 v2。',
        codexIdentityPolicyV1: 'v1（传统粗粒度策略）',
        codexIdentityPolicyV2: 'v2（精细化会话隔离与 Wire-ID，强一致）',
        codexClientProfile: '客户端档案',
        codexClientProfileDesc: '配置与上游交互时的协议画像。系统将自动对齐对应的客户端特征、Wire-ID 规范及协议降级处理。',
        codexClientProfileAuto: 'Auto（动态根据请求推导）',
        codexClientProfilePassthrough: 'Passthrough（调用方透传）',
        codexClientProfileCodexCli: 'Codex CLI（官方命令行画像 0.148.0）',
        codexClientProfileCodexExec: 'Codex Exec（官方后台执行画像 0.148.0）',
        codexClientProfileCodexDesktop: 'Codex Desktop（官方桌面应用画像 0.148.0）',
        codexClientProfileOpencode: 'OpenCode（第三方 IDE 插件画像 1.2.4）',
        codexClientProfilePi: 'Pi Coding Agent（通用编码代理画像）',
        codexProfileAppVersion: '配置版本',
        codexProfileCapabilities: '协议能力',
        codexProfileFidelity: '协议一致性',
        codexProfilePending: '待解析',
        ...
        codexClientConfiguration: '客户端配置',
        codexPresetClient: '客户端身份',
        codexPresetManaged: '预设托管',
        codexPresetCustom: '自定义（保留现有）',
        codexPresetCustomize: '自定义配置',
        codexPresetVersionPolicy: '版本策略',
        codexPresetSystemVersion: '跟随系统版本',
        codexPresetPinnedVersion: '固定兼容版本',
        codexPresetAutoVersion: '自动同步兼容版本',
        codexPresetHeldVersion: '已手动固定',
        codexUpdatePending: '等待检查官方稳定版。',
        codexUpdateCurrent: '兼容版本已同步。',
        codexUpdateNeedsReview: '发现稳定版 {version}，请求实现或依赖需要适配；当前版本保持不变。',
        codexUpdateFailed: '版本检查失败，当前版本保持不变。',
        codexUpdateHeld: '自动更新已暂停，当前版本已固定。',
        codexUpdateStorageUnavailable: '版本存储暂不可用；已加载的有效配置保持不变。',
        codexPresetDiagnostics: '诊断详情',
        codexPresetCompatibility: 'HTTP/SSE 兼容档案；原生 TLS/H2 一致性未验证。',
        codexPresetLegacySwitch: '保存将切换旧账号的执行路径；未固定的旧会话需要先结束。',
        codexManagementMode: '识别策略',
        codexProfileExplicit: '指定客户端',
        codexClientFamily: '客户端家族',
        codexAdvancedConfiguration: '高级配置与兼容选项',
        codexProfileVariant: '版本与变体',
        codexProfileUnknown: '当前版本不支持此历史档案；原值未被自动替换。',
        codexCatalogUnavailable: '档案目录不可用，能力状态未知。',
        codexPreviewUnavailable: '有效配置预览不可用；尚未核验。',
        codexPreviewOperation: '操作',
        codexPreviewTransport: '传输',
        codexEffectiveSender: '预计发送器',
        codexBundleTLSConflict: '共享 r1 档案与 TLS 指纹模拟冲突，要求显式关闭 TLS 指纹模拟。',
        codexBundleBulkTLSRequired: '批量应用 r1 必须同时明确关闭 TLS 指纹模拟；未修改不等于已关闭。',
        codexBulkPreviewPending: '批量 TLS 设置未指定，各账号有效传输待核验。',
        codexPluginPreviewPending: '插件归属尚未核验，内置发送器并未获得运行时确认。',
        codexManagedDefaultGated: '新建受管默认未开启：全局密钥、分布式注册表与安装迁移尚未联合验收。',
        codexExistingPinsRetained: '已固定会话保留原身份和传输；未固定旧会话不自动迁移。',
        ...
        codexProfileUnverified: '未验证',
        codexProfileDegraded: '存在差异',
        codexProfileSupported: '支持',
        codexProfileUnsupported: '不支持',
        codexRelayShadowEnabled: 'Shadow 对照观测',
      ...
      groupPriorityTitle: '分组内调度优先级',
      groupPriorityHint: '数值越小，账号在该 OpenAI 分组中的调度层级越高。保存时使用当前优先级进行冲突检查。',
        ...
        openai: {
          ...
          errors: {
            ...
            OPENAI_EGRESS_PROXY_REQUIRED:
              '当前 OpenAI 出口策略要求代理，但此账号未绑定代理；请绑定代理，或在配置中使用 optional 模式允许直连。',
            OPENAI_EGRESS_PROXY_UNAVAILABLE:
              'OpenAI 出口代理不可用或配置无效；请检查代理状态、有效期、协议、主机和端口。',
            OPENAI_EGRESS_PROXY_INVALID:
              'OpenAI 出口代理配置无效；请检查协议、主机、端口和 fallback 设置。'
          },
          // Refresh Token auth
