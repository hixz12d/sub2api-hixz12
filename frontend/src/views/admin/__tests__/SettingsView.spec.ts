...
const refreshPaymentConfig = vi.hoisted(() => vi.fn().mockResolvedValue(null));
vi.mock("@/stores/payment", () => ({ usePaymentStore: () => ({ fetchConfig: refreshPaymentConfig }) }));
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("saves the purchase entry switches and refreshes linked visibility", async () => {
    getSettings.mockResolvedValue({ ...baseSettingsResponse, payment_enabled: true,
      payment_purchase_entry_enabled: true, payment_purchase_entry_perpay_linked: false });
    const wrapper = mountView();
    await flushPromises();
    await openPaymentTab(wrapper);
    const enabled = wrapper.get<HTMLInputElement>('[data-testid="purchase-entry-enabled"]');
    const linked = wrapper.get<HTMLInputElement>('[data-testid="purchase-entry-perpay-linked"]');
    expect(enabled.element.checked).toBe(true);
    expect(linked.element.checked).toBe(false);
    await linked.setValue(true);
    await enabled.setValue(false);
    expect(linked.attributes('disabled')).toBeDefined();
    await wrapper.find('form').trigger('submit.prevent');
    await flushPromises();
    expect(updateSettings).toHaveBeenCalledWith(expect.objectContaining({
      payment_purchase_entry_enabled: false, payment_purchase_entry_perpay_linked: true,
    }));
    expect(refreshPaymentConfig).toHaveBeenCalledWith(true);
    wrapper.unmount();
  });
