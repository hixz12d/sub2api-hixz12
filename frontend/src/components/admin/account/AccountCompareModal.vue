        ...
        <article v-for="card in cards" :key="card.id" :data-account-id="card.id" class="min-w-0 overflow-hidden rounded-xl border border-gray-200 dark:border-dark-600">
          <header class="border-b border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
            ...
            <div class="mt-1 flex flex-wrap justify-between gap-2 text-xs text-gray-500">
              <span>{{ card.actualModel || card.requestModel || model }}</span>
...
interface Card {
  ...
  requestModel: string; actualModel: string; requestPrompt: string
...
function newCard(id: number): Card {
  return { id, name: props.knownAccounts.find(a => a.id === id)?.name || `#${id}`, status: 'waiting', answer: '', error: '', requestModel: '', actualModel: '', requestPrompt: '', startedAt: 0, finishedAt: 0, recordId: '', verdict: 'unlabeled', reason: '', revision: 0, saving: false, reviewError: '', reviewSaved: false }
}
function stop() {
  generation++
...
async function runCard(card: Card, run: number, requestModel: string, requestPrompt: string) {
  Object.assign(card, { status: 'running', answer: '', error: '', actualModel: '', requestModel, requestPrompt, startedAt: Date.now(), finishedAt: 0, recordId: '', revision: 0, verdict: 'unlabeled', reason: '', reviewError: '', reviewSaved: false })
...
async function runBatch(targets: Card[], requestModel: string, requestPrompt: string) {
  ...
  for (const card of targets) Object.assign(card, newCard(card.id), { name: card.name, requestModel, requestPrompt })
  const worker = async () => {
    while (generation === run && next < targets.length) await runCard(targets[next++], run, requestModel, requestPrompt)
  }
...
function startAll() { void runBatch(cards.value, model.value.trim(), prompt.value) }
function retry(card: Card) { void runBatch([card], card.requestModel || model.value.trim(), card.requestPrompt || prompt.value) }
